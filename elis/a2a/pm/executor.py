...
Governed A2A message semantics: Gate 2E + Gate 3 policy validation via
``validate_message()``.  Enforces sender role check, timestamp governance,
self-target rejection, and structured rejection outcomes.
"""
...
class PMExecutor(AgentExecutor):
    """
    ELIS PM executor — official SDK pattern with governed message semantics.
    ...
    On ``execute``:
      ...
      4. Validate incoming message metadata via ``validate_message()``.
         Reject if sender role, timestamp, or message type policy fails.
      5. Use ``TaskUpdater`` for lifecycle updates — ack on success.
    ...
    async def execute(
        ...
        logger.info(
            "PMExecutor.execute called — task_id=%s context_id=%s",
            task_id,
            context_id,
        )
        ...
        if task is None:
            ...
            logger.debug("PMExecutor: enqueued new Task id=%s", task_id)
        ...
        if not result.allowed:
            ...
            logger.info(
                "PMExecutor: rejected — code=%s task_id=%s",
        ...
        ack_text = (
            "ELIS PM: acknowledgement received via A2A channel. "
        ...
        logger.info(
            "PMExecutor.execute complete — task_id=%s",
            task_id,
        )
    ...
    async def cancel(
        ...
        context_id: str = context.context_id  # type: ignore[assignment]
        logger.info(
            "PMExecutor.cancel called — task_id=%s",
            task_id,
        )
        task_updater = TaskUpdater(event_queue, task_id, context_id)

"""
ELIS Supervisor — Agent Executor.
...
class SupervisorExecutor(AgentExecutor):
    """
    ELIS Supervisor executor — official SDK pattern.
    ...
    async def execute(
        ...
        logger.info(
            "SupervisorExecutor.execute called — task_id=%s context_id=%s",
            task_id,
            context_id,
        )
        ...
        if task is None:
            ...
            logger.debug("SupervisorExecutor: enqueued new Task id=%s", task_id)
        ...
        if not result.allowed:
            ...
            logger.info(
                "SupervisorExecutor: rejected — code=%s task_id=%s",
        ...
        ack_text = (
            "ELIS Supervisor: acknowledgement received via A2A channel. "
        ...
        logger.info(
            "SupervisorExecutor.execute complete — task_id=%s",
            task_id,
        )
    ...
    async def cancel(
        ...
        context_id: str = context.context_id  # type: ignore[assignment]
        logger.info(
            "SupervisorExecutor.cancel called — task_id=%s",
            task_id,
        )
        task_updater = TaskUpdater(event_queue, task_id, context_id)

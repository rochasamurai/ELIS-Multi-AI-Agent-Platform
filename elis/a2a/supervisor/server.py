"""
ELIS Supervisor — Server wiring.

Assembles the ASGI application for the ELIS Supervisor A2A server using the
...
from elis.a2a.supervisor.agent_card import SUPERVISOR_RPC_PATH, build_agent_card
from elis.a2a.supervisor.executor import SupervisorExecutor
...
def run(host: str = "127.0.0.1", port: int = 9501) -> None:
    """
    Start the ELIS Supervisor A2A server on localhost.
    ...
    Args:
        ...
        port: TCP port number (default 9501).
    """
    if host != "127.0.0.1":
        raise ValueError(
    ...
    logger.info("Starting ELIS Supervisor A2A server on %s:%d", host, port)
